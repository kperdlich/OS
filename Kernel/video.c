...
static void putC(char character, uInt8_t color, uInt32_t memory)
{
    char* memoryPtr;
    memoryPtr = (char*)memory;
    *memoryPtr = character;
    memoryPtr++;
    *memoryPtr = color;
}
...
void k_print(const char* string, uInt8_t color, Console* pos)
{
    const char* tmpString = (const char*)string;
    while (*tmpString != '\0') {
...
void putChar(const char string, uInt8_t color, Console* pos)
{
    uInt32_t memoryPtr = calc_vPtrForConsole(pos);
