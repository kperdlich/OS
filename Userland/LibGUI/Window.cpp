...
void Window::onMouseMove(int x, int y)
{
    ...
    if (IntRect { m_rect.x() + m_rect.width() - TitleBarHeight, m_rect.y(), TitleBarHeight, TitleBarHeight }.isPointInside(x, y)) {
        m_closeButton->onMouseMove(x, y);
        return;
    }

    if (IntRect { m_rect.x() + m_rect.width() - (TitleBarHeight * 2), m_rect.y(), TitleBarHeight, TitleBarHeight }.isPointInside(x, y)) {
...
{
    if (m_rect.isPointInside(x, y) && m_isDragging) {
        m_isDragging = false;
        return;
    }
...
void Window::onMouseDown(int key, int x, int y)
{
    ...
    if (titleBarRect().isPointInside(x, y)) {
        m_lastMouseMovePos = { x, y };
        m_isDragging = true;
        return;
    }
